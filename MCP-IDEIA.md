# Ideia de Servidor MCP — EQ17

**Domínio:** Gestão de repúblicas universitárias (despesas)  
**Data:** 2026-07-01
...
## Servidor proposto: `republica-mcp`

### Tools sugeridas

- `registrar_despesa(casaId, valor, tipo)` — lança despesa
- `dividir_despesas(casaId, mes)` — rateio do mês
- `saldo_morador(userId)` — quanto cada um deve
- `notificar_moradores(casaId, msg)` — aviso

### Resources (somente leitura)

- extrato da casa como resource

### Exemplos de uso com um LLM

- "Quanto eu devo esse mês e quem ainda não pagou?"
...
public class RepublicaTools {
    ...
    public RepublicaTools(SeuService seuService) { this.seuService = seuService; }

    @Tool(description = "lança despesa")
    public Object registrar_despesa(/* params */) {
